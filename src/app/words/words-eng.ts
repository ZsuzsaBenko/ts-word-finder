import { Words } from '../models';

export const wordsEng: Words = {
    'A': {
        '3': [
            'ACE', 'AGE', 'AIM', 'ALE', 'ALL', 'AND', 'ANT', 'APP', 'ARE', 'ART', 'ASK', 'AYE'
        ],
        '4': [
            'ABLE', 'ACHE', 'ACID', 'AMID', 'ANEW', 'ANON', 'AREA', 'ATOM', 'AUNT'
        ],
        '5': [
            'ABASE', 'ABASH', 'ABORT', 'ABOUT', 'ABYSS', 'ACTOR', 'ADMIT', 'ADORE', 'AGENT', 'AGILE', 'ALARM', 'ALERT', 'ALONE', 'ALTER',
            'AMBER', 'AMEND', 'AMITY', 'AMPLE', 'ANGER', 'ANGRY', 'ARISE', 'AROMA', 'ARROW', 'ASSET', 'ATONE', 'AVOID', 'AWARE'
        ],
        '6': [
            'ABDUCT', 'ABOARD', 'ABSURD', 'ACCENT', 'ACIDIC', 'ADMIRE', 'ADVERB', 'ADVERT', 'ADVICE', 'ADVISE', 'AGENCY', 'AMIDST', 'AMOUNT',
            'ANIMAL', 'ANEMIC', 'ANOINT', 'ANSWER', 'AROUND', 'AROUSE', 'ARTIST', 'ASSERT', 'ATOMIC', 'AUGUST', 'AUTUMN'
        ],
        '7': [
            'ABANDON', 'ACTRESS', 'ACQUIRE', 'ADDRESS', 'ADVANCE', 'ADVERSE', 'ADVISOR', 'AGILITY', 'ANALOGY', 'ANALYSE', 'ANALYZE',
            'ANATOMY', 'ANOMALY', 'ANOTHER'
        ]
    },
    'B': {
        '3': [
            'BAD', 'BAN', 'BAT', 'BED', 'BEG', 'BIG', 'BIT', 'BOX', 'BUG', 'BUY'
        ],
        '4': [
            'BABE', 'BABY', 'BACK', 'BAIL', 'BAKE', 'BALL', 'BATH', 'BELL', 'BEAK', 'BEAM', 'BEAN', 'BEAR', 'BIKE', 'BILE', 'BILL', 'BIND',
            'BIRD', 'BITE', 'BLUE', 'BOAR', 'BODY', 'BOLD', 'BOMB', 'BONE', 'BOOK', 'BOOT', 'BORN', 'BUCK', 'BULL', 'BUTT'
        ],
        '5': [
            'BATHE', 'BLACK', 'BLAME', 'BLEED', 'BLOCK', 'BLOOD', 'BOARD', 'BOOTS', 'BRAIN', 'BREAD', 'BREAK', 'BREED', 'BRIBE', 'BRIDE',
            'BROAD', 'BROOD', 'BROWN', 'BRUSH', 'BUILD', 'BUNNY', 'BUYER'
        ],
        '6': [
            'BANDIT', 'BANKER', 'BARREN', 'BEGGAR', 'BLOODY', 'BORROW', 'BREATH', 'BRIDGE', 'BUDGET', 'BUDGIE', 'BUTTER'
        ],
        '7': [
            'BELIEVE', 'BISCUIT', 'BLISTER', 'BRACKET', 'BREATHE', 'BROTHEL', 'BROTHER'
        ]
    },
    'C': {
        '3': [
            'CAB', 'CAR', 'CAT', 'CRY', 'CUT'
        ],
        '4': [
            'CALM', 'CAMP', 'CARD', 'CARE', 'CARP', 'CART', 'CASE', 'CAST', 'CLAN', 'CHEW', 'CHIN', 'CHOP', 'CODE', 'COOK', 'CORE', 'COST',
            'CRAM', 'CREW', 'CUTE'
        ],
        '5': [
            'CANDY', 'CARGO', 'CARRY', 'CATCH', 'CHAIN', 'CHART', 'CHECK', 'CHICK', 'CHIEF', 'CHILD', 'CLASS', 'CLAIM', 'CLEAN', 'CLEAR',
            'CLOAK', 'CLOCK', 'CLOSE', 'CLOUD', 'COLOR', 'COUGH', 'COVER', 'CRACK', 'CRAFT', 'CRAMP', 'CREAM', 'CROSS', 'CURSE'
        ],
        '6': [
            'CARROT', 'CATCHY', 'CHANGE', 'CHARGE', 'CLEVER', 'CLIENT', 'CLOUDY', 'COLOUR', 'COLUMN', 'COMMON', 'COOKIE', 'COSTLY',
            'COUGAR', 'COUPLE', 'COURSE', 'CRAMPS', 'CREATE', 'CRINGE', 'CRINGY', 'CURSED'
        ],
        '7': [
            'CAPTURE', 'CHECKED', 'CHICKEN', 'CLASSIC', 'CLOTHES', 'COMMENT', 'COMPASS', 'CONSENT', 'CONTROL', 'CRICKET', 'CRINGEY',
            'CRUMBLE', 'CUTLERY'
        ]
    },
    'D': {
        '3': [
            'DAD', 'DAM', 'DAY', 'DIE', 'DIM', 'DOG', 'DOT', 'DRY'
        ],
        '4': [
            'DALE', 'DAMN', 'DAMP', 'DARN', 'DATA', 'DATE', 'DEAD', 'DEAL', 'DEAR', 'DEER', 'DIME', 'DINE', 'DISC', 'DIVE', 'DOOR', 'DOWN',
            'DRUG', 'DRUM', 'DUCK', 'DUST'
        ],
        '5': [
            'DADDY', 'DAILY', 'DAIRY', 'DAISY', 'DANCE', 'DEATH', 'DEMON', 'DEVIL', 'DINER', 'DREAM', 'DRINK', 'DRIVE', 'DROWN', 'DUSTY',
            'DWELL'
        ],
        '6': [
            'DAMNED', 'DANCER', 'DEADLY', 'DELETE', 'DEMAND', 'DESIGN', 'DETAIL', 'DIGEST', 'DISARM', 'DOCTOR', 'DOTTED', 'DOUBLE',
            'DRAMA', 'DREAMY', 'DRIVER'
        ],
        '7': [
            'DAYTIME', 'DEATHLY', 'DENTIST', 'DEPRESS', 'DIAMOND', 'DIVORCE', 'DRUMMER', 'DUSTBIN'
        ]
    },
    'E': {
        '3': [
            'EAR', 'EAT', 'EEL', 'EGO', 'END', 'EYE'
        ],
        '4': [
            'EARL', 'EARN', 'EAST', 'EDIT', 'EVEN', 'EVER', 'EXAM', 'EXIT'
        ],
        '5': [
            'EARLY', 'EARTH', 'EIGHT', 'ELECT', 'ELOPE', 'ENTER', 'EQUAL', 'ERASE', 'ERECT', 'EVENT', 'EVERY', 'EXACT'
        ],
        '6': [
            'EARTHY', 'EGOIST', 'ELAPSE', 'ELEVEN', 'EMPLOY', 'ENDING', 'ENDURE', 'ENERGY', 'ERASER', 'ESCAPE', 'EVOLVE', 'EXCEPT',
            'EXCITE', 'EXPECT', 'EXPERT'
        ],
        '7': [
            'EARNEST', 'EARSHOT', 'EARTHLY', 'EASTERN', 'ELASTIC', 'ELEGANT', 'ELEMENT', 'ELEVATE', 'ENDLESS', 'EVENING', 'EXACTLY',
            'EXAMPLE', 'EXHAUST', 'EXPLAIN'
        ]
    },
    'F': {
        '3': [
            'FAN', 'FAT', 'FIG', 'FIR', 'FIT', 'FIX', 'FLY', 'FOX', 'FRY', 'FUN'
        ],
        '4': [
            'FACE', 'FAIL', 'FAIR', 'FALL', 'FAME', 'FAST', 'FEED', 'FEEL', 'FEND', 'FILE', 'FILM', 'FIND', 'FINE', 'FIRE', 'FIRM', 'FISH',
            'FIVE', 'FLAT', 'FLIP', 'FOLD', 'FOLK', 'FOND', 'FOOD', 'FORD', 'FORK', 'FOUL', 'FOUR', 'FRET', 'FROG', 'FULL', 'FUND'
        ],
        '5': [
            'FAIRY', 'FAITH', 'FALSE', 'FATTY', 'FAVOR', 'FELON', 'FENCE', 'FETCH', 'FEVER', 'FIELD', 'FIEND', 'FIERY', 'FIFTY', 'FIRST',
            'FLUTE', 'FORCE', 'FORTY', 'FOYER', 'FRONT', 'FOUND', 'FUNNY'
        ],
        '6': [
            'FAMILY', 'FAMINE', 'FAMOUS', 'FAVOUR', 'FATHER', 'FEMALE', 'FELONY', 'FIGURE', 'FILTER', 'FINGER', 'FLINCH', 'FLOWER',
            'FOLLOW', 'FRIDAY'
        ],
        '7': [
            'FAILURE', 'FALSIFY', 'FEELING', 'FENCING', 'FIREFLY', 'FLOWERY', 'FORESEE', 'FOREVER', 'FORGIVE', 'FORWARD', 'FOUNDER',
            'FRECKLE', 'FULFILL'
        ]
    },
    'G': {
        '3': [
            'GAP', 'GEL', 'GET', 'GIN', 'GIT', 'GOD', 'GUN'
        ],
        '4': [
            'GAIN', 'GALE', 'GAME', 'GAZE', 'GEAR', 'GIST', 'GIVE', 'GLAD', 'GLEE', 'GLOW', 'GOAL', 'GOAT', 'GOTH', 'GRAN', 'GRAY', 'GREY',
            'GROW'
        ],
        '5': [
            'GABLE', 'GAMER', 'GHOST', 'GIANT', 'GLARE', 'GLASS', 'GLEAM', 'GLINT', 'GLOOM', 'GLORY', 'GOOSE', 'GRACE', 'GRADE', 'GRAND',
            'GRAVE', 'GRAVY', 'GREAT', 'GREEN', 'GREET', 'GROUP', 'GROWL'
        ],
        '6': [
            'GAMBLE', 'GARAGE', 'GARLIC', 'GENIUS', 'GINGER', 'GLANCE', 'GLASSY', 'GLOBAL', 'GLOOMY', 'GLOVES', 'GOTHIC', 'GRANNY',
            'GREASE', 'GREASY', 'GROWTH'
        ],
        '7': [
            'GENERAL', 'GLAMOUR', 'GLEEFUL', 'GLIMPSE', 'GLORIFY', 'GODDESS', 'GRANDMA'
        ]
    },
    'H': {
        '3': [
            'HAD', 'HAG', 'HAM', 'HAS', 'HAT', 'HAY', 'HEN', 'HER', 'HIS', 'HIT', 'HOE', 'HOG', 'HOT', 'HUB', 'HUG'
        ],
        '4': [
            'HAIL', 'HAIR', 'HALF', 'HALL', 'HANG', 'HARD', 'HARE', 'HATE', 'HAVE', 'HEAD', 'HEAL', 'HEAR', 'HELL', 'HELP', 'HERE', 'HERS',
            'HIGH', 'HIND', 'HINT', 'HIRE', 'HOLD', 'HOLE', 'HOLY', 'HOME', 'HOPE', 'HORN', 'HOUR', 'HUGE'
        ],
        '5': [
            'HAIRY', 'HAPPY', 'HAVEN', 'HEART', 'HEAVE', 'HEAVY', 'HEDGE', 'HELLO', 'HINGE', 'HOLLY', 'HONEY', 'HORSE', 'HOUSE', 'HUSKY'
        ],
        '6': [
            'HARDLY', 'HEADER', 'HEALTH', 'HEAVEN', 'HEIGHT'
        ],
        '7': [
            'HAIRCUT', 'HAIRPIN', 'HARMONY', 'HEALTHY', 'HELPFUL', 'HOPEFUL', 'HUNDRED'
        ]
    },
    'I': {
        '3': [
            'INK', 'IRE'
        ],
        '4': [
            'INKY', 'IRON', 'ITEM'
        ],
        '5': [
            'IMAGE', 'INNER', 'IRATE', 'IRONY'
        ],
        '6': [
            'IGNORE', 'INDEED', 'INDUCE', 'INGEST', 'INJURE', 'INJURY', 'INSERT', 'INVEST', 'INSANE', 'IREFUL'
        ],
        '7': [
            'ILLEGAL', 'IMAGINE', 'INCLUDE', 'INGRESS', 'INSPIRE', 'INSTANT', 'INTEREST'
        ]
    },
    'J': {
        '3': [
            'JAM'
        ],
        '4': [
            'JOIN'
        ],
        '5': [
            'JEANS', 'JOINT'
        ],
        '6': [
            'JUNGLE'
        ],
        '7': ['JUNIPER']
    },
    'K': {
        '3': [
            'KEY'
        ],
        '4': [
            'KEEP', 'KILL', 'KING', 'KNEE', 'KNOW'
        ],
        '5': [
            'KNEEL', 'KNIFE'
        ],
        '6': [
            'KILLER', 'KITTEN', 'KNIGHT'
        ],
        '7': [
            'KITCHEN'
        ]
    },
    'L': {
        '3': [
            'LAB', 'LAD', 'LAX', 'LAY', 'LET', 'LIE', 'LIP', 'LOG'
        ],
        '4': [
            'LACK', 'LACE', 'LACY', 'LADY', 'LAKE', 'LAMB', 'LAME', 'LAMP', 'LAND', 'LANE', 'LARD', 'LARK', 'LASH', 'LATE', 'LEAN', 'LEND',
            'LIAR', 'LICK', 'LIFE', 'LIKE', 'LINE', 'LIMP', 'LINK', 'LINT', 'LION', 'LIST', 'LIVE', 'LOCK', 'LOOK', 'LONG', 'LORD', 'LOSE',
            'LOUD', 'LOVE', 'LUCK', 'LYNX'
        ],
        '5': [
            'LABEL', 'LABOR', 'LANCE', 'LARGE', 'LARVA', 'LASER', 'LASSO', 'LATCH', 'LATER', 'LAYER', 'LEARN', 'LEAVE', 'LEGAL', 'LIMIT',
            'LIVER', 'LOBBY', 'LODGE', 'LOOSE', 'LOSER', 'LOUSE', 'LOUSY', 'LOVER', 'LUCKY'
        ],
        '6': [
            'LABOUR', 'LANCET', 'LAPTOP', 'LARYNX', 'LATENT', 'LATEST', 'LATTER', 'LENGTH', 'LETHAL', 'LINDEN', 'LINGER', 'LINTEL',
            'LISTEN', 'LOCKER', 'LONELY', 'LOUNGE'
        ],
        '7': [
            'LANTERN', 'LATERAL', 'LATTICE', 'LETTUCE', 'LIBRARY', 'LOBSTER'
        ]
    },
    'M': {
        '3': [
            'MAN', 'MAP', 'MAT', 'MAY', 'MID', 'MOM', 'MOP', 'MUD', 'MUG', 'MUM'
        ],
        '4': [
            'MAID', 'MAIL', 'MALE', 'MASK', 'MASS', 'MATE', 'MEAL', 'MEAN', 'MEAT', 'MEET', 'MERE', 'MESS', 'MILD', 'MILE', 'MILK', 'MIND',
            'MINE', 'MINT', 'MISS', 'MOCK', 'MODE', 'MORE', 'MOTH', 'MOVE', 'MUCK', 'MUST'
        ],
        '5': [
            'MAGIC', 'MARRY', 'MATCH', 'MEDIA', 'METAL', 'MICRO', 'MIDST', 'MIGHT', 'MINCE', 'MINER', 'MODEL', 'MONEY', 'MOUND', 'MOUNT',
            'MOUSE', 'MOVIE', 'MUCKY', 'MUDDY', 'MUMMY', 'MUSIC'
        ],
        '6': [
            'MAIDEN', 'MARVEL', 'MASTER', 'MATTER', 'MEMBER', 'MENTAL', 'MIGHTY', 'MINCER', 'MOMENT', 'MONDAY', 'MOTHER', 'MURDER'
        ],
        '7': [
            'MACHINE', 'MAGICAL', 'MARRIED', 'MEANING', 'MESSAGE', 'MINDFUL', 'MINERAL', 'MOCKERY', 'MORNING', 'MUSICAL'
        ]
    },
    'N': {
        '3': [
            'NAP', 'NAY', 'NET', 'NEW', 'NOD', 'NOT', 'NUN', 'NUT'
        ],
        '4': [
            'NAIL', 'NAME', 'NAVY', 'NECK', 'NEWS', 'NINE', 'NOTE', 'NOUN'
        ],
        '5': [
            'NAPPY', 'NEVER', 'NIGHT', 'NOVEL'
        ],
        '6': [
            'NINETY', 'NOBODY', 'NUMBER'
        ],
        '7': [
            'NIGHTLY', 'NOTHING', 'NOWHERE', 'NUCLEAR'
        ]
    },
    'O': {
        '3': [
            'OAK', 'OFF', 'ONE', 'OUR', 'OUT', 'OWL'
        ],
        '4': [
            'OBEY', 'ONCE', 'ONLY', 'OPEN', 'OURS', 'OVER'
        ],
        '5': [
            'OAKEN', 'OFFER', 'OPTION', 'ORDER', 'OTHER', 'OUGHT', 'OUNCE'
        ],
        '6': [
            'OCCUPY', 'OFFEND', 'OFFICE', 'ONLINE', 'ONWARD'
        ],
        '7': [
            'OFFICER', 'OFFLINE', 'ONGOING'
        ]
    },
    'P': {
        '3': [
            'PAY', 'PIN', 'PIT', 'POP', 'PUB', 'PUN', 'PUT'
        ],
        '4': [
            'PACK', 'PAGE', 'PAIR', 'PART', 'PATH', 'PEND', 'PICK', 'PINK', 'PLAY', 'POEM', 'POET', 'POND', 'POOR', 'PULL', 'PUMP', 'PUNK'
        ],
        '5': [
            'PAINT', 'PAPER', 'PARTY', 'PAUSE', 'PHONE', 'PIANO', 'PLACE', 'PLAIN', 'PLANT', 'PLUCK', 'POINT', 'POUND', 'POWER', 'PRIDE',
            'PRINT', 'PROUD', 'PRUNE', 'PULSE', 'PUNCH', 'PUPPY'
        ],
        '6': [
            'PALACE', 'PARENT', 'PLAYER', 'PENCIL', 'PEOPLE', 'PERIOD', 'PERSON', 'PIRATE', 'POCKET', 'POINTY', 'POLICY', 'POTION',
            'PUBLIC', 'PUNISH'
        ],
        '7': [
            'PAINTER', 'PATTERN', 'PENDANT', 'PERFECT', 'PICTURE', 'PREMIUM', 'PREPARE', 'PRIMARY', 'PRINTER', 'PRIVACY', 'PROCESS',
            'PRODUCT', 'PROFILE', 'PROJECT', 'PROMOTE', 'PUBLISH', 'PUNGENT'
        ]
    },
    'Q': {
        '3': [],
        '4': [
            'QUAD', 'QUAY', 'QUIT', 'QUIZ'
        ],
        '5': [
            'QUEEN', 'QUEER', 'QUERY', 'QUEST', 'QUEUE', 'QUICK', 'QUIET', 'QUIRK', 'QUITE', 'QUOTA', 'QUOTE'
        ],
        '6': [
            'QUAINT', 'QUARRY'
        ],
        '7': [
            'QUALITY', 'QUARREL', 'QUARTER'
        ]
    },
    'R': {
        '3': [
            'RAM', 'RAP', 'RAT', 'RAW', 'RAY', 'RED', 'RID', 'ROB', 'ROW', 'RUG', 'RUM', 'RUN'
        ],
        '4': [
            'RACE', 'RACK', 'RAIN', 'RAPE', 'RARE', 'RASH', 'RATE', 'RAVE', 'RENT', 'REST', 'RICH', 'RIDE', 'RING', 'RISE', 'ROAM', 'ROCK',
            'ROLL', 'ROOM', 'ROSE', 'RUDE', 'RULE', 'RUNE', 'RUSH'
        ],
        '5': [
            'RADAR', 'RADIO', 'RAINY', 'RAISE', 'RAZOR', 'READY', 'REBEL', 'RENEW', 'REPLY', 'RIVER', 'ROBIN', 'ROGUE', 'ROUND', 'ROUTE',
            'ROYAL', 'RULER'
        ],
        '6': [
            'RABBIT', 'RARELY', 'RATHER', 'REBOOT', 'RECIPE', 'REMIND', 'RELATE', 'REMOVE', 'RENAME', 'RENDER', 'RESIST', 'REVEAL',
            'ROBBER', 'RUNNER', 'RHYTHM'
        ],
        '7': [
            'RAINBOW', 'RECEIPT', 'REGULAR', 'REVERSE', 'ROBBERY'
        ]
    },
    'S': {
        '3': [
            'SAP', 'SAY', 'SEA', 'SET', 'SEX', 'SIN', 'SIR', 'SIX', 'SIT', 'SKY', 'SUM', 'SUN'
        ],
        '4': [
            'SACK', 'SAFE', 'SAIL', 'SALT', 'SAME', 'SALE', 'SAND', 'SANE', 'SAVE', 'SEAL', 'SEAT', 'SEEK', 'SELF', 'SELL', 'SEND', 'SHOW',
            'SHUT', 'SICK', 'SIGN', 'SING', 'SINK', 'SIRE', 'SKIN', 'SLAM', 'SNOW', 'SOAP', 'SOFT', 'SOME', 'SONG', 'SOUP', 'SOUR', 'SPIT',
            'STAG', 'STAR', 'STEW', 'STOP', 'SUIT'
        ],
        '5': [
            'SABER', 'SAINT', 'SALTY', 'SAPPY', 'SAVOR', 'SAVVY', 'SCARE', 'SCARF', 'SCARY', 'SCREW', 'SERVE', 'SEVEN', 'SHAME', 'SHARK',
            'SHARP', 'SHELL', 'SHINE', 'SHIFT', 'SHINE', 'SHIRE', 'SHIRT', 'SHOCK', 'SHOES', 'SHOOT', 'SHORT', 'SILLY', 'SIXTY', 'SLACK',
            'SLAVE', 'SLEEP', 'SLIDE', 'SLIME', 'SLIMY', 'SMART', 'SMELL', 'SMILE', 'SNAKE', 'SNEAK', 'SNOWY', 'SOCKS', 'SOLID', 'SOUND',
            'SPELL', 'SPICE', 'SPICY', 'SPOON', 'SPRAY', 'STAGE', 'STAIN', 'STALE', 'START', 'STATE', 'STEAL', 'STEAM', 'STEEL', 'STING',
            'STICK', 'STING', 'STINK', 'STOCK', 'STORE', 'STORM', 'STUDY', 'STYLE', 'SUNNY', 'SWEAR', 'SWEET', 'SWING'
        ],
        '6': [
            'SAILOR', 'SAMPLE', 'SAVAGE', 'SAVIOR', 'SAVOUR', 'SAYING', 'SCARED', 'SCHOOL', 'SCREAM', 'SCREEN', 'SEARCH', 'SEASON',
            'SECOND', 'SELECT', 'SELFIE', 'SENDER', 'SHIVER', 'SHORTS', 'SHOULD', 'SHOWER', 'SICKEN', 'SILVER', 'SINGER', 'SINGLE',
            'SISTER', 'SKINNY', 'SLEEPY', 'SLOPPY', 'SMOOTH', 'SNEEZE', 'SOURCE', 'SPIRIT', 'SPRING', 'STARRY', 'STATUS', 'STINKY',
            'STREAM', 'STRONG', 'STUPID', 'SUMMER', 'SUNDAY', 'SWITCH', 'SYSTEM'
        ],
        '7': [
            'SAVIOUR', 'SERVICE', 'SETTING', 'SEVENTY', 'SHELTER', 'SHOOTER', 'STATION', 'STOMACH', 'STRANGE', 'STUPEFY', 'SUCCESS',
            'STYLISH'
        ]
    },
    'T': {
        '3': [
            'TAN', 'TAP', 'TEA', 'TEN', 'THE', 'TIN', 'TIT', 'TOE', 'TOP', 'TRY', 'TWO'
        ],
        '4': [
            'TAIL', 'TAKE', 'TALE', 'TAME', 'TANK', 'TAPE', 'TASK', 'TEAL', 'TEAM', 'TENT', 'THEM', 'THEN', 'THIN', 'TILE', 'TILL', 'TIME',
            'TOOL', 'TRAM', 'TRAY', 'TREE', 'TRIP', 'TRUE', 'TUBE', 'TUNE', 'TYPE'
        ],
        '5': [
            'TABLE', 'TALON', 'TASTE', 'TEACH', 'THANK', 'THEFT', 'THEIR', 'THERE', 'THICK', 'THIEF', 'THING', 'THINK', 'THORN', 'THREE',
            'THROW', 'TIGER', 'TIMER', 'TIRED', 'TODAY', 'TOOTH', 'TOUCH', 'TRACE', 'TRAIL', 'TRAIN', 'TRAMP', 'TREND', 'TROOP', 'TRUCK',
            'TRUTH', 'TWICE'
        ],
        '6': [
            'THANKS', 'THEIRS', 'THINGY', 'THIRTY', 'THRIVE', 'TOILET', 'TOMATO', 'TOUCHY', 'TRENDY', 'TWELVE', 'TWENTY'
        ],
        '7': [
            'TEACHER', 'TERRIER', 'THOUGHT', 'TODDLER', 'TONIGHT', 'TRAINER', 'TROOPER', 'TUESDAY'
        ]
    },
    'U': {
        '3': [
            'URN', 'USE'
        ],
        '4': [
            'UNIT', 'URGE', 'USER'
        ],
        '5': [
            'UNCLE', 'UNDER', 'UNFIT', 'UNITE', 'UNTIE', 'UNTIL', 'UPPER', 'USAGE'
        ],
        '6': [
            'UNABLE', 'UNCLEAN', 'UNCLEAR', 'UNFAIR', 'UNFURL', 'UPDATE'
        ],
        '7': [
            'UNAWARE', 'UNCANNY', 'UNCOVER', 'UNDERGO'
        ]
    },
    'V': {
        '3': [
            'VAN', 'VET', 'VOW'
        ],
        '4': [
            'VALE', 'VAMP', 'VANE', 'VARY', 'VASE', 'VAST', 'VERB', 'VERY'
        ],
        '5': [
            'VAGUE', 'VALID', 'VALOR', 'VALUE', 'VALVE', 'VIDEO', 'VIRAL', 'VIRUS', 'VISIT', 'VOWEL'
        ],
        '6': [
            'VACUUM', 'VALLEY', 'VALOUR', 'VANISH', 'VANITY', 'VERIFY', 'VERSUS', 'VIOLIN'
        ],
        '7': [
            'VAGRANT', 'VALIANT', 'VAMPIRE', 'VARIANT', 'VARIOUS', 'VISIBLE'
        ]
    },
    'W': {
        '3': [
            'WAN', 'WAR', 'WAS', 'WAY', 'WET', 'WHY', 'WIG', 'WIN'
        ],
        '4': [
            'WAIL', 'WAIT', 'WANE', 'WANT', 'WARD', 'WARM', 'WARN', 'WASH', 'WAVE', 'WAVY', 'WEAK', 'WEAN', 'WEAR', 'WELL', 'WERE', 'WHAT', 'WHEN',
            'WHOM', 'WILL', 'WIND', 'WING', 'WIRE', 'WISH', 'WITH', 'WORD', 'WORK'
        ],
        '5': [
            'WATER', 'WAIST', 'WEARY', 'WEAVE', 'WHERE', 'WHICH', 'WHILE', 'WHITE', 'WHOLE', 'WHOSE', 'WIDOW', 'WIELD', 'WINCE', 'WINDY', 'WITCH',
            'WOMAN', 'WORLD', 'WOULD', 'WOUND', 'WRING', 'WRITE', 'WRONG'
        ],
        '6': [
            'WAITER', 'WEALTH', 'WEAPON', 'WEASEL', 'WEIGHT', 'WINDOW', 'WINNER', 'WINTER', 'WITHER', 'WRAITH', 'WRITER', 'WRITHE'
        ],
        '7': [
            'WARLOCK', 'WARLORD', 'WARNING', 'WEALTHY', 'WEATHER', 'WISHFUL', 'WHISPER', 'WOUNDED'
        ]
    },
    'Y': {
        '3': [
            'YEA', 'YES', 'YET', 'YOU'
        ],
        '4': [
            'YEAH', 'YEAR', 'YELL', 'YELP', 'YOUR', 'YUCK'
        ],
        '5': [
            'YEARN', 'YEAST', 'YIELD', 'YOUNG', 'YOURS', 'YUMMY'
        ],
        '6': [
            'YEARLY', 'YELLOW'
        ],
        '7': []
    },
    'Z': {
        '3': [
            'ZOO'
        ],
        '4': [
            'ZANY', 'ZEAL', 'ZERO', 'ZONE', 'ZOOM'
        ],
        '5': [],
        '6': [
            'ZEALOT', 'ZENITH', 'ZIGZAG', 'ZODIAC'
        ],
        '7': []
    }
};
